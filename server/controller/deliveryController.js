...
const { Devilvery } = require("../../models/index.js");
...
const getOne = async (req, res) => {
  const { id } = req.params;
  const data = await Devilvery.findOne({ where: { id: id } });
  if (data) {
    Devilvery.findOne({
      where: {
        id: id,
    ...
    res.send("BU ID boyuncha Devilvery yok!");
  }
};
...
const update = async (req, res) => {
  ...
  const data = await Devilvery.findOne({ where: { id: id } });
  if (!data) {
    res.json("Bu Id boyuncha Devilvery yok!");
  } else {
    Devilvery.update(
      {
        name_tm,
...
const Destroy = async (req, res) => {
  const { id } = req.params;
  const data = await Devilvery.findOne({ where: { id } });
  if (data) {
    Devilvery.destroy({
      where: {
        id,
