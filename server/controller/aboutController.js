...
const getOne = async (req, res) => {
  ...
  if (data) {
    ...
    res.send("BU ID boyuncha About yok!");
  }
};
...
const update = async (req, res) => {
  ...
  const data = await About.findOne({ where: { id: id } });
  if (!data) {
    res.json("Bu Id boyuncha About yok!");
  } else {
    About.update(
      {
        name_tm,
...
const Destroy = async (req, res) => {
  const { id } = req.params;
  const data = await About.findOne({ where: { id } });
  if (data) {
    About.destroy({
      where: {
        id,
